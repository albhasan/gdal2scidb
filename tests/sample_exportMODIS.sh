...
parallel --jobs 0 --eta python $SCRIPT_FOLDER/gdal2bin_chunk.py --d2tid true --d2att false --tile2id false --output $OUTPUT --log error {1} {2} $CHUNKSIZE $CHUNKSIZE $FIRST_CID $FIRST_RID $FILES ">" $OUT_FOLDER/mod13q1_h"$H"v"$V"_{1}_{2} ::: $(seq 0 $CHUNKSIZE $LAST_ID) ::: $(seq 0 $CHUNKSIZE $LAST_ID)
